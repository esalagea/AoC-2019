...
def manhatan_dist(p1, p2, q1, q2):
    return abs(p1 - q1) + abs(p2 - q2)


def closest_intersection(i, j, board):
    min_dist = 100000000;

    for key in board:
        if board[key] == 3:
            dist = manhatan_dist(i, j, key[0], key[1])
            if dist < min_dist:
               min_dist = dist
    return min_dist


def visit_horisontal(board, i, j, steps, wireNb, direction, current_step, intersections):
    for step in range(steps):
        j = j + direction
        current_step = current_step + 1
        if board[(i, j)] == 0 or board[(i, j)] == wireNb:
            board[(i, j)] = wireNb
        else:
            board[(i, j)] = 3
            intersections[i, j] = intersections[i, j] + current_step
    return ((i, j), current_step)


def visit_vertical(board, i, j, steps, wireNb, direction, current_step, intersections):
    for step in range(steps):
        i = i + direction
        current_step = current_step + 1
        if board[(i, j)] == 0 or board[(i, j)] == wireNb:
...
def visitBoard(board, i, j, wire, wireNb, intersections):
    current_step = 0
    pos = (i, j)
    for entry in wire:
        direction = entry[0]
        steps = int(entry[1:])
        if direction == "R":
            pos, current_step = visit_horisontal(board, pos[0], pos[1], steps, wireNb, 1,  current_step, intersections)
        if direction == "L":
            pos, current_step = visit_horisontal(board, pos[0], pos[1], steps, wireNb, -1,  current_step, intersections)
        if direction == "D":
            pos, current_step = visit_vertical(board, pos[0], pos[1], steps, wireNb, 1,  current_step, intersections)
        if direction == "U":
            pos, current_step = visit_vertical(board, pos[0], pos[1], steps, wireNb, -1,  current_step, intersections)
...
# sc = 0
#
# w1s = "R75,D30,R83,U83,L12,D49,R71,U7,L72"
# w1 = w1s.split(",")
#
# w2s = "U62,R66,U55,R34,D71,R55,D58,R83"
# w2 = w2s.split(",")

visitBoard(map, sl, sc, w1, 1, intersections)
visitBoard(map, sl, sc, w2, 2, intersections)
visitBoard(map, sl, sc, w1, 1, intersections)



print closest_intersection(sl, sc, map)
print intersections

print min(intersections.items(), key=lambda x: x[1])

# visitBoard(map, wire1, 1)
